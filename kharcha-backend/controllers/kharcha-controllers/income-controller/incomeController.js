...
import { getFormatedDate } from "../../../utilities/utility.js";
...
export const addIncome = asyncHandler(async (req, res) => {
  ...
  if (typeof amount !== "number" || isNaN(amount)) {
    return res
      .status(400)
      .json(getErrorResponse("Amount must be a valid number."));
  }
...
export const getIncome = asyncHandler(async (req, res) => {
  ...
  const defaultEndDate = endDateParam ? new Date(endDateParam) : currentDate;
  ...
  if (defaultStartDate > maxStartDate) {
    return res
      ...
      .json(
        getErrorResponse(
          "startDate cannot exceed 31st December of the previous year 23:59:59 hrs."
        )
      );
  }
  ...
  try {
    ...
    const formattedIncomeData = incomeData.map((item) => ({
      ...
      date: getFormatedDate(item.createdOn), // Format the date as "DD, Month"
    }));
